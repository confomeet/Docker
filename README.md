...
3. Создание образов контейнеров docker.
...
Сначала скачивается репеозиторий.
...
Затем необходимо сконфигурировать систему с помощью переменных окружения, рекоммендуется скопировать файл _example.env_
