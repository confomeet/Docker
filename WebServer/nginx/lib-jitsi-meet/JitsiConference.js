 ...
 * @param {string} id The participant id.
 */
JitsiConference.prototype.lobbyKickBack = function(id) {
    logger.log(`Attempting to send user (${id}) back to lobby`);
    if (!this.room) {
        logger.error(`Attempt to send user (${id}) back to lobby failed (No room)`);

        return;
    }

    const lobbyRoom = this.room.getLobby();
    if (!lobbyRoom) {
        logger.error(`Attempt to send user (${id}) back to lobby failed (No lobby)`);

        return;
    }


    lobbyRoom.kickBackToLobby(id);
}
/**
