capabilities.json stores the capabilities of Lilac Meet per browser per version of the browser. The JSON structure is the following:
```
{
...
The capabilities property of `CapabilitiesByVersion` object stores the capabilities for the use case where Lilac Meet is not loaded in an iframe. For the use case when Lilac Meet is loaded in an iframe the capabilities are calculated by `Object.assign(capabilities, iframeCapabilities)`.
