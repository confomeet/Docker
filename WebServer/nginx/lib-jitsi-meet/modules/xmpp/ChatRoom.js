...
export default class ChatRoom extends Listenable {
    ...
    onMessage(msg, from) {
        ...
        const kickBackToLobby = $(msg).find('>kick-back-to-lobby').text();
        if (kickBackToLobby) {
            const lobbyRoom = $(msg).find('>lobbyroom').text();

            this.eventEmitter.emit(XMPPEvents.KICK_BACK_TO_LOBBY_RECEIVED, lobbyRoom);
        }
    }
    ...
    onPresenceError(pres, from) {
        ...
        logger.warn('onPresError ', pres);
        this.eventEmitter.emit(
            XMPPEvents.ROOM_CONNECT_DUPLICATE_ACCESS_ERROR);
