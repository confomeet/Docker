...
local function verify_user(session, stanza)
    log("info", "Session token: %s, session room: %s, stanza: %s",
    ...
    if string.match(tostring(stanza), "recorder@recorder") then
        return true;
    end

    if string.match(tostring(stanza), "jigasi@auth.") then
        return true;
    end
